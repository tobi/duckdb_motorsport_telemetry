...
The browser build accepts `.pds`, `.ld`, and `.vbo` files only. AiM `.mp4`
support is native DuckDB-only in v0.7.0 because the WASM adapter deliberately
rejects the memory-mapped `aimd` reader; use `read_aim` from a native DuckDB
installation for MP4 recordings.
