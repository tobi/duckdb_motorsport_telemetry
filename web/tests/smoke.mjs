...
writeFileSync(vbo, `[header]\ntime\nvelocity kmh\nthrottle\nlap\nlatitude\nlongitude\nlap distance\n[column names]\ntime velocity throttle lap latitude longitude lapdistance\n[data]\n120000.0 10 0 0 41.0000 2.0000 0\n120010.0 20 10 0 41.0010 2.0010 50\n120020.0 30 20 1 41.0020 2.0020 0\n120030.0 40 30 1 41.0030 2.0020 25\n120040.0 50 40 1 41.0040 2.0010 50\n120050.0 60 50 1 41.0040 2.0000 75\n120100.0 70 60 2 41.0030 1.9990 0\n120110.0 80 70 2 41.0020 1.9980 50\n120120.0 90 80 2 41.0010 1.9980 90\n120130.0 60 70 3 41.0000 1.9990 0\n120140.0 40 40 3 40.9995 2.0000 50\n120150.0 20 10 3 41.0000 2.0000 90\n`);
await page.locator('#fileInput').setInputFiles(vbo);
await page.waitForFunction(() => document.querySelector('#fileFormat')?.textContent === 'VBO' && document.querySelectorAll('#channelRows tr').length === 7, null, { timeout: 30_000 });
...
await page.waitForFunction(() => !document.querySelector('#scrubber')?.classList.contains('hidden') && document.querySelector('#scrubValues')?.textContent?.includes('DELTA') && document.querySelector('#comparisonStatus')?.textContent?.toLowerCase().includes('lap distance'));
await page.locator('#trackMap').hover({ position: { x: 300, y: 100 } });
await page.waitForFunction(() => !document.querySelector('#mapTooltip')?.classList.contains('hidden') && document.querySelector('#mapTooltip')?.textContent?.includes('SPEED'));
...
await page.waitForFunction(() => document.querySelector('#queryTiming')?.textContent?.includes('7 ROWS'), null, { timeout: 30_000 });
