...
writeFileSync(vbo, `[header]\ntime\nvelocity kmh\nthrottle\n[column names]\ntime velocity throttle\n[data]\n120000.0 10 0\n120000.5 20 50\n120001.0 30 100\n`);
await page.locator('#fileInput').setInputFiles(vbo);
await page.waitForFunction(() => document.querySelector('#fileFormat')?.textContent === 'VBO' && document.querySelectorAll('#channelRows tr').length === 3, null, { timeout: 30_000 });
await page.click('#runQuery');
await page.waitForFunction(() => document.querySelector('#queryTiming')?.textContent?.includes('3 ROWS'), null, { timeout: 30_000 });
