...
writeFileSync(vbo, `[header]\ntime\nvelocity kmh\nthrottle\nlap\n[column names]\ntime velocity throttle lap\n[data]\n120000.0 10 0 0\n120010.0 20 10 0\n120020.0 30 20 1\n120030.0 40 30 1\n120040.0 50 40 1\n120050.0 60 50 1\n120100.0 70 60 2\n120110.0 80 70 2\n120120.0 90 80 2\n120130.0 60 70 3\n120140.0 40 40 3\n120150.0 20 10 3\n`);
await page.locator('#fileInput').setInputFiles(vbo);
await page.waitForFunction(() => document.querySelector('#fileFormat')?.textContent === 'VBO' && document.querySelectorAll('#channelRows tr').length === 4, null, { timeout: 30_000 });
await page.waitForFunction(() => document.querySelectorAll('#lapRail button').length === 4 && document.querySelector('#lapRail button.active')?.textContent?.includes('BEST') && document.querySelectorAll('#recipeGrid button').length === 10, null, { timeout: 30_000 });
...
await page.waitForFunction(() => document.querySelector('#queryTiming')?.textContent?.includes('4 ROWS'), null, { timeout: 30_000 });
